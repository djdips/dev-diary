...
const routes: Record<string, Record<string, RouteHandler>> = {
    GET: {
        ...
        "/post/:slug": (req, params) =>
            withAuth(req, () => getPost(req, params)),
        "/post/:slug/meta": (req, params) =>
            withAuth(req, () => getPostMeta(req, params)),
        "/tag/:tagName": (req, params) =>
            withAuth(req, () => getPostsByTag(req, params)),
        "/search": (req, params) =>
            withAuth(req, () => searchPosts(req, params)),
    },
    POST: {
        "/post": (req, params) => withAuth(req, () => createPost(req, params)),
        "/login": login,
        "/refresh-token": refreshTokenHandler, // Assuming refreshToken is handled in login
    },
    PUT: {
        "/post/:slug": (req, params) =>
            withAuth(req, () => editPostBySlug(req, params)),
    },
    DELETE: {
        "/post/:slug": (req, params) =>
            withAuth(req, () => deletePost(req, params)),
    },
}
...
export async function handleRequest(req: Request): Promise<Response> {
    ...
    for (const pattern in methodRoutes) {
        ...
        if (match) {
            ...
            return await methodRoutes[pattern](req, {
